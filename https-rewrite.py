...
# This file can be safely removed by running: sudo rm /opt/bitnami/lightsail/scripts/https-rewrite.py

# To enable HTTP to HTTPS rewrite, run the following commands in a SSH session:
# sudo python3 /opt/bitnami/lightsail/scripts/https-rewrite.py WebsiteSetupLECert
...
def main():
    if len(argv) == 1:
        print("usage: sudo <python/python3> https-rewrite.py <target_cert_name>")
